...
/*
    ...
    Update4: Let's say there are multiple cars waiting for refuling and the fuel refill rate is increased such that two cars can get fuel
    at the same time (at a point there is enough fuel for 2 cars). We observe a problem here with pthread_cond_signal here since this function
    only guarantees that atleast one thread waiting on the conditional variable will be unblocked (no guarantee of all).

    Update5: To unblock all the threads waiting on the conditional variable, we should use pthread_cond_brodcast rather than pthread_cond_signal.
    Now we can also scale to two fuel refill threads and still as soon as fuel is enough for multiple cars, all of them will take it.
*/
